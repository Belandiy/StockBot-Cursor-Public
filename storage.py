...
def remove_tracking(chat_id: int, tracking_type: str, ticker: str, *params):
    ...
    # 2. Удаление из active_trackings в памяти
    try:
        ...
        # Удаляем конкретный тип отслеживания с проверкой параметров
        if tracking_type == "regular" and "regular" in ticker_data and len(params) > 0:
            ...
            if interval_key in ticker_data["regular"]:
                ...
                # Если не осталось регулярных отслеживаний для этого тикера
            ...
            if follow_key in ticker_data["follow"]:
                ...
                # Если не осталось пороговых отслеживаний для этого тикера
